...
import { HonorTile, DragonTile, WindTile } from './tile-classes'
...
export function calculateFu (hand) {
  ...
  var pinfu = new Pinfu()
  var isChiiToitsu = chiiToitsu.check(hand)
  var isPinfu = pinfu.check(hand)
  ...
  for (let i = 0; i < hand.openCombinaisons.length; i++) {
    if (hand.openCombinaisons[i] instanceof Pon) {
      const firstTile = hand.openCombinaisons[i].tiles[0]
  ...
  for (let i = 0; i < hand.concealedCombinaisons.length; i++) {
    if (hand.concealedCombinaisons[i] instanceof Pon) {
      const firstTile = hand.concealedCombinaisons[i].tiles[0]
      if (firstTile instanceof HonorTile || firstTile.isTerminal()) {
  ...
  for (let i = 0; i < hand.combinaisons.length; i++) {
    if (hand.combinaisons[i] instanceof Pair) {
      const firstTile = hand.combinaisons[i].tiles[0]

...
describe('given the pair is both the seat and round wind', () => {
  ...
  test('should attribute 4 fu for the pair', () => {
    ...
    const result = sut.calculate(hand)
    expect(result.details.find(x => x.key === 'pair')).toStrictEqual({ key: 'pair', fuValue: 2, quantity: 2 })
  })
})
